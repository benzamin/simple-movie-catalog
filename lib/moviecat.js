...
function active_sort(id, dir)
{
    ...
    sort_dir.innerHTML = (dir > 0) ? "<font color='red'>&#9650;</font>" : "<font color='red'>&#9660;</font>";
...
function do_filter()
{
    ...
    var my, mr, mrt, mtags, movieTitle, moviePlot;
    var show;
    ...
    for (i=0; i<rows.length; i++) {
        ...
        if(searchText !== ''){
          movieTitle = getValue(rows[i],"MTITLE").toLowerCase();
          moviePlot = getValue(rows[i],"plot").toLowerCase();
          var fullText = movieTitle + ' ' + my + ' ' + mr + ' ' + mg.toLowerCase() + ' ' + moviePlot;
          show = !!~fullText.indexOf(searchText);
        }
